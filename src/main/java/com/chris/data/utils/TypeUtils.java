package com.chris.data.utils;

import com.chris.data.annotation.Expand;
import com.chris.data.annotation.query.Query;
import com.chris.data.model.Self;
...
public class TypeUtils {
     ...
     * @return
     */
    public static Map<String, com.chris.data.model.Column> getColumnNameMapFromBaseClass(Class<?> baseClazz) {
        if (baseClazz == null) {
            return null;
        }
        Map<String, com.chris.data.model.Column> colnumNameMap = new HashMap<>();
        //获取所有字段
        ...
        try {
            ...
            for (Field field : fields) {
                ...
                if (!StringUtils.isEmpty(colName1)) {
                    colnumNameMap.put(fieldName, new com.chris.data.model.Column(colName1, typeName));
                } else {
                    Method writeMethod = pd.getWriteMethod();
                    String colName2 = getColumnNameFromMethod(writeMethod);
                    //如果从setter方法上面找到了
                    if (!StringUtils.isEmpty(colName2)) {
                        colnumNameMap.put(fieldName, new com.chris.data.model.Column(colName2, typeName));
                    } else {
                        //获取
                        ...
                        if (column != null) {
                            ...
                            if (colName != null) {
                                colnumNameMap.put(fieldName, new com.chris.data.model.Column(colName, typeName));
                            }
                        }
                    }
                }
